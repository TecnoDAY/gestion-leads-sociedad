-- Fix RPCs for the deployed leads schema, which uses "Agente" without a trailing space.
begin;
...
comment on function public.create_lead(jsonb) is 'Creates a lead using the deployed Agente column without a trailing space.';
comment on function public.update_lead_full(bigint, jsonb) is 'Admin full edit using the deployed Agente column without a trailing space.';
